...
void loop() {
  if (cfg.deviceType == 1 && camCfg.enabled) {
    processCameraLogic();  // State machine runs here
  }
  
  if (cameraLogicActive) {
    delay(5);  // Don't sleep while active
    return;
  }
...
struct CameraConfig {
  ...
  uint8_t inputActivePolarity;          // Reserved (0=activeLow)
  uint8_t outputDriveMode;              // Reserved (0=openDrain)
...
**Exit to CAM_IDLE:** Z elapsed and activity has ended
...
- Release HP only after Z and either cap reached or `wakeHoldDeadlineMs` expired
...
- Sleep 950ms between 50ms advertisements
- Average: ~150-200µA

**Active camera logic:**
- `cameraLogicActive = true`
- Main loop skips sleep: `delay(5)` only
...
case CAM_IDLE:
  cameraLogicActive = false;
  if (camCfg.powerSaveIdleMode) return;  // Skip all processing
  // ...
```

This saves power by not even checking for triggers when idle. Triggers still fire (ISR sets flags), but state machine doesn't process them until mode is disabled.
...
1. State machine in `processCameraLogic()`, called from `loop()`
