...
def _sequence_starts(fp_out_active: list[int]) -> list[int]:
    ...
    threshold = max(500, int(baseline * 2.5))
...
def extract_metrics(edges: list[EdgeRecord], run_id: str = "unknown") -> dict[str, float | int | bool | str | None]:
    ...
    fp_spacings = _spacings(fp_out_a)
    ...
    if fp_out_i:
        ...
        hp_release_before_final_frame_detected = any(t < final_fp_release for t in hp_out_i)
    ...
    wake_only_hold = None
    if hp_out_a and hp_out_i and not fp_out_a:
        wake_only_hold = float(hp_out_i[-1] - hp_out_a[0])
    ...
    out: dict[str, Any] = {
        ...
        "fpInToHpOutLatencyMs": _latency(fp_in_a, hp_out_a),
        "fpInToFpOutLatencyMs": _latency(fp_in_a, fp_out_a),
        "firstFrameAfLeadMs": _latency(hp_out_a, fp_out_a),
        ...
        "hpOutContinuityMs": float(hp_out_i[-1] - hp_out_a[0]) if hp_out_a and hp_out_i else None,
