...
|--------|-------------|-------------------|
| `hpInToHpOutLatencyMs` | First valid HP IN active edge to HP OUT active edge | SC-01, SC-04, SC-12, SC-15 P1 |
...
| `hpHoldAfterLastFrameMs` | Last FP OUT release to HP OUT release | SC-01, SC-05, SC-05b |
...
Pulse HP IN at t=0, then again before the hold expires. With `wakeHoldRefreshPolicy = extend`, HP OUT must remain active and the timeout must move later from each valid HP edge.
...
| `hpHoldAfterLastFrameMs` |  |  |  |  | +/-1% or +/-10 ms |  |
...
4. All failures are linked to a bug, documentation correction, or accepted product decision.
5. The final report includes raw captures and exact parameter readbacks for every case.
